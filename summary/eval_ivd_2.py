...
OUTPUT_FILE = 'summary2_ivd_results.csv'
STATS_FILE  = 'summary2_ivd_stats.csv'
...
def main():
    ...
    for in_file in tqdm(
        ...
        if processed and processed % 10 == 0:
            ...
            tmp.to_csv(f'summary2_ivd_results_{processed}.csv', index=False)
            avg = tmp.groupby('file_id').mean(numeric_only=True).reset_index()
            avg.to_csv(f'summary2_ivd_stats_{processed}.csv', index=False)
            pd.DataFrame(error_log).to_csv(f'summary2_ivd_errors_{processed}.csv', index=False)
    ...
    if all_results:
        ...
        pd.DataFrame(error_log).to_csv('summary2_ivd_errors_final.csv', index=False)
